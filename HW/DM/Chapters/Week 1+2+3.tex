...
\end{exercise}
\begin{proof}
    任取 \(  p \in M  \), 设 \(  U_{p}\subseteq M  \)是开集, 使得 \(  f\equiv c  \) 在 \(  U_{p}  \)上成立. 存在光滑的 bump函数 \(   \lambda \in C^{\infty}\left( M \right)   \), 使得 \[
     \lambda \left( p \right)= 1,\quad \operatorname{supp}\left(  \lambda  \right)\subseteq U_{p}, \quad 0\le  \lambda \left( q \right)\le 1,\forall q\in M   
    \]   由于 \(  X_{p}c= 0  \), 且 \(  X_{p}  \)是线性的, 因此 \(  X_{p}f= 0  \)当且仅当 \(  X_{p}\left( f-c \right)= 0   \), 因此不妨设 \(  c= 0  \).  此时在 \(  U_{p}  \)上, 由于 \(  f= 0  \), \(   \lambda f= 0  \), 在       \(  M\setminus \operatorname{supp} \lambda   \)上, 由于 \(   \lambda = 0  \), \(   \lambda f= 0  \).   由于 \(  M\subseteq U\cup \left( M\setminus \operatorname{supp} \lambda  \right)   \),  \(   \lambda f  \equiv 0\). 由导子性 \[
    0= X_{p}\left(  \lambda f \right)=  \lambda \left( p \right)\left( X_{p}f \right)+ f\left( p \right) X_{p} \lambda = X_{p}f      
    \]  
\end{proof}
\begin{exercise}{}{}
    若 \(   \varphi :M\to N  \)是微分同胚, 证明 \(  \operatorname{dim}M= \operatorname{dim}N  \).  
\end{exercise}
\begin{proof}
    设 \(  p \in M  \), \(  q=  \varphi \left( p \right)\in N   \). 则由于 \(   \varphi   \)是微分同胚, \[
    \mathrm{Id}_{T_{p}M}= d\left( \mathrm{Id}_{M} \right)_{p}= d\left(  \varphi ^{-1}  \right)_{q} \circ d\left(  \varphi  \right)   _{p}
    \]   因此 \[
    d \varphi _{p}: T_{p}M\to T_{q}N
    \]是一个线性同构. 故 \(  \operatorname{dim}T_{p}M  = \operatorname{dim}T_{q}N\).  只需要说明 \(  \operatorname{dim}T_{p}M= \operatorname{dim}M  \), \(  \operatorname{dim}T_{q}N= \operatorname{dim}N  \)   . 设 \(  m= \operatorname{dim}M  \). 考虑以 \(  p  \)为中心的坐标卡 \(  \left( \psi= \left( x^{1},\cdots ,x^{m} \right)  ,U  \right)   \)  . 断言 \[
    T_{p}M= \operatorname{span}\left\{ \left. \frac{\partial }{\partial x^{1}} \right|_{p},\cdots ,\left. \frac{\partial }{\partial x^{m}} \right|_{m} \right\}
    \]其中 \[
    \left. \frac{\partial }{\partial x^{i}} \right|_{p}\left( f \right) =\left.  \frac{\partial \left( f\circ \psi  ^{-1}  \right) }{\partial r^{i}} \right|_{ \psi  \left( p \right) }
    \]为了说明线性无关, 注意到 \[
    \left. \frac{\partial }{\partial x^{i}} \right|_{p}\left( x^{j} \right) =  \delta _{i}^{j}
    \]于是 \[
    \sum _{i= 1}^{m}c^{m}\left. \frac{\partial }{\partial x^{m}} \right|_{p}= 0\implies  c^{i} \delta _{i}^{j}= 0\implies c^{i}= 0,\forall i
    \]为了说明 这组向量张成了 \(  T_{p}M  \), 断言 \(  v= \sum _{i= 1}^{m}v\left( x^{i} \right)\left. \frac{\partial }{\partial x^{i}} \right|_{p}   \) , 令 \(  \hat{f}= f\circ  \varphi ^{-1}   \)  , 则由Taylor展开,存在光滑函数 \(  G_{ij}  \), 使得 \[
    ...
    \]代入 \(  y=  \psi  \left( s\right)   \), 则 \[
    f\left( s \right)= f\left( p \right)+ \sum _{i= 1}^{m}\left( x^{i}\left( s \right)-x^{i}\left( p \right)   \right)\left. \frac{\partial f}{\partial x^{i}} \right|_{p}   
    \] 则 \[
    v\left( f \right)= v\left( f\left( p \right)+ \sum _{i= 1}^{m}\left( x^{i}-x^{i}\left( p \right)  \right)g_{i}  \right)  = v\left( x^{i} \right)g_{i}\left( p \right)= \sum _{i= 1}^{m}v\left( x^{i} \right)\left. \frac{\partial f}{\partial x^{i}} \right|_{p}   
    \]其中\(  g_{i}\left( p \right)= \left. \frac{\partial f}{\partial x^{i}} \right|_{p}   \) . 因此 \(  \operatorname{dim}T_{p}M= m= \operatorname{dim}M  \). 类似地, 可以说明 \(  \operatorname{dim}T_{q}N= \operatorname{dim}N  \)  .
\end{proof}

\begin{exercise}{}{}
    考虑 \[
    \det : \operatorname{GL} \left( n,\mathbb{R}  \right)\to \mathbb{R}  
    \]\begin{enumerate}
        \item 证明 \(  \det   \)光滑;
        \item 对于 \(  A \in \operatorname{GL} \left( n,\mathbb{R}  \right)   \), 计算 \(  T_{A}\operatorname{GL} \left( n,\mathbb{R}  \right)   \)
        \item 计算 \(  \left( d \det  \right)_{A}   \)    
    \end{enumerate}
    
\end{exercise}
\begin{proof}
  \begin{enumerate}
    \item   考虑 \(  \det :M\left( n,\mathbb{R}  \right)\to \mathbb{R}    \) \[
    \det \left( A \right)= \sum _{ \sigma \in S_{n}} \mathrm{sgn}\left(  \sigma  \right)\prod _{i= 1}^{n}a_{i, \sigma \left( i \right) } 
    \]是 \( \mathbb{R} ^{n^{2}}\simeq  M\left( n,\mathbb{R}  \right)\to \mathbb{R} ^{2}   \)  的多项式函数, 进而是连续且光滑的. 我们有 \[
    \operatorname{GL} \left( n,\mathbb{R}  \right)=  \left( \det  \right)^{-1} \left(\mathbb{R} \setminus 0 \right)   
    \]是一个开子集, 从而有自然的继承自 \(  M\left( n,\mathbb{R}  \right)\simeq \mathbb{R} ^{n^{2}}   \)的光滑结构. 在这个光滑结构下, \(  \det : \operatorname{GL} \left( n,\mathbb{R}  \right)\to \mathbb{R}    \)的光滑性等价于 \(  \det : M\left( n,\mathbb{R}  \right)\to \mathbb{R}    \)限制在 \(  \operatorname{GL} \left( n,\mathbb{R}  \right)   \)上的光滑性, 自然是成立的.
    \item 由于 \(  M\left( n,\mathbb{R}  \right)\simeq \mathbb{R} ^{n^{2}}   \), 由于 \(  T_{p}\mathbb{R} ^{n^{2}}  \)到 \(  \mathbb{R} ^{n^{2}}  \)有自然的同构 \( \left.  \frac{\partial }{\partial x^{i}}   \right|_{p}  \mapsto \varepsilon^{i}\) . 设 \(  p  \)是 \(  A  \)同构到 \(  \mathbb{R} ^{n^{2}}  \)中的点, 则    \[
    T_{A}M\left( n,\mathbb{R}  \right) \simeq T_{p}\mathbb{R} ^{n^{2}}\simeq \mathbb{R} ^{n^{2}}\simeq M\left( n,\mathbb{R}  \right) 
    \]  具体地, \(  T_{A}M\left( n,\mathbb{R}  \right)   \)通过将 \(  \left. \frac{\partial }{\partial x_{i,j}}  \right|_{A} \) 映到 \(E_{i,j}  \)  同构于 \(  M\left( n,\mathbb{R}  \right)   \) 
    \item \[
    \left( d \det  \right)_{A}: M\left( n,\mathbb{R}  \right)\simeq  T_{A}\operatorname{GL} \left( n,\mathbb{R}  \right)\to   T_{\det \left( A \right) }\mathbb{R} \simeq \mathbb{R} 
    \]计算 \(  \left( d \det  \right)_{A}   \), 相当于计算一个 \(  \mathbb{R} ^{n^{2}}  \)到 \(  \mathbb{R}   \)欧式导数, 一个数分 结论是 \[
    \frac{\partial \det A}{  \partial a_{kl}}= C_{kl}
    \]其中 \(  C_{kl}  \)是 \(  a_{kl}  \)的代数余子式.   
        因此 \[
    \left( d \det  \right)_{A}\left( B \right)= \sum _{k= 1}^{n}\sum _{l = 1}^{n}C_{kl}b_{kl}  = \sum _{k= 1}^{n}\sum _{l = 1}^{n}\left( A^{*} \right)_{lk}B_{kl} = \sum _{k= 1}^{n}\left( A^{*}\cdot B \right)_{kk}=  \mathrm{tr}\left( A^{*}\cdot B \right) 
      \]又 \[
      A^{-1} = \frac{1 }{\det A } A^{*} 
      \]我们有 \[
...
\begin{exercise}{}{}
    设 \[
    F:M\to N
    \]是光滑函数, \(  p \in M  \), \(  \left( dF \right)_{p}   \)是同构. 证明 \(  F  \)在 \(  p  \)处是局部的微分同胚.    
\end{exercise} 

\begin{proof}
    记 \(  q= f\left( p \right)   \). 取包含 \(  p  \)的坐标卡 \(  \left( U_0, \varphi  \right)   \)和包含了 \(  q  \)的坐标卡 \(  \left( V_0,\psi  \right)   \). 不妨设      \(  F\left( U_0 \right)\subseteq V_0   \). 定义  \[
    f\coloneqq \psi \circ F\circ  \varphi ^{-1} :  \varphi \left( U_0 \right)\to \psi \left( V_0 \right)  
    \]设 \(  x=  \varphi \left( p \right)   \).  \[
    \,\mathrm{d} F_{p}= \left( \,\mathrm{d} \psi ^{-1}  \right)_{f\left( x \right) }\circ \,\mathrm{d} f_{x}\circ \left( \,\mathrm{d}  \varphi  \right)_{p}  
    \]由于 \(  \,\mathrm{d} F_{p}  \)是同构, \(  \det \left( J_{f}\left(  \varphi \left( p \right)  \right)  \right)\neq 0   \). 由关于 \(  f  \)的反函数定理, 存在开集 \(  A  \)使得 \(   \varphi \left( p \right)\in A\subseteq  \varphi \left( U_0 \right)    \), 满足
    \begin{itemize}
        \item \(  B= f\left( A \right)   \)是 \(  \mathbb{R} ^{n}  \)  上的开集;
        \item \(  f|_{A}:A\to B  \)是微分同胚
    \end{itemize}
         令 \(  U=  \varphi ^{-1} \left( A \right)   \), 则 \(  U  \)是 \(  M  \)上的开集, 使得 \(  p \in U  \). 令 \(  V= \psi ^{-1} \left( B \right)   \), 则 \(  V  \)是 \(  N  \)上的开集, 使得 \(  q \in V  \). 因此 \(  F|_{U}:U\to V  = \psi ^{-1} \circ f|_{A}\circ  \varphi \)是一个微分同胚.         
\end{proof}
\end{document}