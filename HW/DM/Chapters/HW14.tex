...
\end{proof}
\begin{exercise}{}{}
设 $M_1,M_2$ 是维数 $n\geqslant 3$ 的连通光滑流形, 令 $M_1\#M_2$ 为它们的\dfntxt{光滑连通和}. \dfntxt{光滑连通和}: 对 $i=1,2$, 令 $U_i$ 是以点 $p_i\in M_i$ 为中心, 同胚于欧氏空间的邻域, 并设 $M_i'=M_i\setminus U_i$, 选取微分同胚 $\varphi:\partial M_1'\to\partial M_2'$, 沿边界粘贴 $M_1',M_2'$ 得到的光滑流形 $M_1\#M_2$.
...
\dfntxt{Lemma : $H^*(M_i)$ 与 $H^*(M_i \setminus \{p_i\})$ 的关系}
...
\begin{itemize}
    \item 当 $0 < p < n-1$ 时, $H^{p-1}(\mathbb{S}^{n-1})=0, H^p(\mathbb{S}^{n-1})=0$. 故 \dfntxt{$H^p(M_i) \cong H^p(M_i \setminus \{p_i\})$}.
    \item 当 $p = n-1$ 时, 序列变为 $0 \to H^{n-1}(M_i) \to H^{n-1}(M_i \setminus \{p_i\}) \xrightarrow{\delta} H^n(D_i)=0$ .  若 $M_i$ 紧致可定向, $H^n(M_i) \cong \mathbb{R}$, 连接同态 $H^{n-1}(\mathbb{S}^{n-1}) \to H^n(M_i)$ 是同构, 迫使前序映射为零. 故此处 \dfntxt{$H^{n-1}(M_i) \cong H^{n-1}(M_i \setminus \{p_i\})$} 依然成立.
...
\dfntxt{b). $p = n-1$ (紧致可定向)}
此时 M-V 序列片段为:
$$H^{n-2}(\mathbb{S}^{n-1}) \to H^{n-1}(M) \to H^{n-1}(U) \oplus H^{n-1}(V) \xrightarrow{\Phi} H^{n-1}(\mathbb{S}^{n-1}) \xrightarrow{\Psi} H^n(M)$$
\begin{itemize}
    \item $H^{n-2}(\mathbb{S}^{n-1}) = 0$.
    \item 因为 $M_1, M_2$ 紧致可定向, $M$ 亦紧致可定向, 故 $H^n(M) \cong \mathbb{R}$. 连接同态 $\Psi: H^{n-1}(\mathbb{S}^{n-1}) \to H^n(M)$ 实际上是将边界的体积元映射为流形的体积元, 这是一个同构.
    \item 因为 $\Psi$ 是同构(单射), 所以 $\operatorname{Im}(\Phi) = \operatorname{Ker}(\Psi) = 0$. 即 $\Phi$ 是零映射.
...
d). 若 $\operatorname{div}_\mu(X)=0$, 则称向量场 $X$ 是\dfntxt{无散的}. 证明: 若 $M$ 是紧致无边流形 (即 $\partial M=\emptyset$), 且 $X$ 是 $M$ 上的无散向量场, 那么对于任意 $f,g\in C^\infty(M)$, 有
